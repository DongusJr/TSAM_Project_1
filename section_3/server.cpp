                  ...
                  char *buffer) 
{
  ...
  std::string commandString;           // Command from the client
  std::string commandOutput;           // Output from the command the client issued
  ...
  if((tokens.size() >= 2) && (tokens[0].compare("SYS") == 0))
  {
    ...
    createCommandStrFromVector(&tokens, &commandString); 
